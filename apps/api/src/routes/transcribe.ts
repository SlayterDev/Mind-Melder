  ...
  fileFilter: (_req, file, cb) => {
    if (ALLOWED_MIME_TYPES.includes(file.mimetype)) {
      ...
      cb(
        new ApiError(
          400,
          `Unsupported audio format: ${file.mimetype}. Supported: mp3, m4a, wav, webm, mp4, ogg, flac`
        )
      );
    }
  },
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!req.file) {
        throw new ApiError(
          400,
          'No audio file provided. Send a multipart form with an "audio" field.'
        );
      }
      ...
      if (!useWhisper && settings.llmProvider !== 'openai') {
        throw new ApiError(
          400,
          `Transcription is not supported by the ${settings.llmProvider} provider. Enable local whisper or switch to OpenAI.`
        );
      }
      ...
      (async () => {
        try {
          console.log(
            `[Transcribe] Starting transcription for user ${userId}, file ${req.file?.originalname}`
          );
          ...
          console.log(
            `[Transcribe] Transcription completed for user ${userId}, file ${req.file?.originalname}`
          );
          ...
          console.log(
            `[Transcribe] Transcription saved for user ${userId}, file ${req.file?.originalname}`
          );
        } catch (err) {
